...
fn main() {
    let values = [("Ketchup", vec!["French Fries", "Burgers", "Hot Dogs"]), ("Mayonnaise", vec!["Sandwiches", "Burgers", "Coleslaw"])];
    let mut sauces_to_meals = HashMap::from(values);

    sauces_to_meals.insert("Mustard", vec!["Hot dog", "Burgers", "Pretzels"]);

    let ans = sauces_to_meals.remove("Mayonnaise");
    let ans_vector = ans.unwrap();

    let mustard = sauces_to_meals.get("Mustard");
    let mustard_vector = mustard.unwrap();
