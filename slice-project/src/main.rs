...
fn main() {
    let mut array = [String::from("Cookie Crisp"), String::from("Cinnamon Toast Crunch"), String::from("Frosted Flakes"), String::from("Cocoa Puffs"), String::from("Captain Crunch") ];

    let first_two = &array[..2];
    println!("{:?}", first_two);

    let mid_three = &array[1..4];
    println!("{:?}", mid_three);

    let last_three = &mut array[2..];
    println!("{:?}", last_three);

    last_three[2] = String::from("Lucky Charms");
    println!("{:?}", array);

    let cookie_crisp = &array[0];

    let cookie = &cookie_crisp[0..6];
    println!("{}", cookie);

    let cocoa_puffs = &array[3];
    // println!("{:?}", cocoa_puffs);
